...
<div class="pr-wrap">
  ...
  <div class="pr-main">
    ...
    function _loadCollections(container) {
        frappe.call({
            method: "competitor_sites_connector.api.pricing.get_collections",
            callback: (r) => {
                ...
                list.querySelectorAll(".pr-collection-item").forEach((el) => {
                    el.addEventListener("click", () => {
                        list.querySelectorAll(".pr-collection-item").forEach(e => e.classList.remove("active"));
...
<div class="pr-collection-item" data-name="${_esc(c.name)}" data-label="${_esc(c.collection_name)}">
  ...
  <div class="pr-collection-meta">${date} · ${c.product_count || 0} products</div>
    ...
    function _loadCollection(container, name, label) {
        ...
        frappe.call({
            method: "competitor_sites_connector.api.pricing.get_collection_items",
...
<tr class="pr-row">
  ...
  <td class="pr-td-source">${_esc(item.source_site || "—")}</td>
    ...
    function _downloadExcel(name, label) {
        ...
        frappe.call({
            method: "competitor_sites_connector.api.pricing.generate_factory_excel",
            args: { collection_name: name },
            callback: (r) => {
                if (btn) { btn.disabled = false; btn.textContent = "⬇ Generate Factory Excel"; }
                const { filename, data } = r.message || {};
                if (!data) { frappe.msgprint("Failed to generate Excel."); return; }
                ...
                a.href = url; a.download = filename || "factory.xlsx";
