...
<div class="rq-wrap">
  ...
  <div class="rq-pagination"></div>
	...
	function _loadProducts(container) {
		...
		frappe.call({
			method: "competitor_sites_connector.api.review_queue.get_review_queue",
			args: {
...
<div class="rq-card ${cls}" data-name="${_esc(p.name)}">
  ...
  <div class="rq-card-body">
	...
	function _setStatus(container, name, status) {
		frappe.call({
			method: "competitor_sites_connector.api.review_queue.set_review_status",
			args: { name, status },
			callback: (r) => {
      ...
      <div class="rq-modal-footer">
		...
		overlay.querySelector(".rq-modal-notes").addEventListener("input", function () {
			...
			notesTimer = setTimeout(() => {
				frappe.call({ method: "competitor_sites_connector.api.review_queue.save_notes", args: { name: p.name, notes: val } });
			}, 800);
		});
	...
	function _finalize(container) {
		...
		const d = new frappe.ui.Dialog({
			...
			primary_action(values) {
				d.hide();
				frappe.call({
					method: "competitor_sites_connector.api.review_queue.finalize_selected",
